let jsonUrl = 'https://kkapi-open-cnortles-action.vercel.app/api/ispeak?author=62e150b2c723ba7be9f4a433' // 在这修改api

document.getElementById('bber-talk').addEventListener('click', () => {
    window.location.pathname = '/talk/' // 在这修改你的哔哔页面地址
})
...
function getData() {
    fetch(jsonUrl)
        ...
        .then((data) => {
            data = {
                time: Date.now(),
                ls: JSON.stringify(data.data.items)
            }
